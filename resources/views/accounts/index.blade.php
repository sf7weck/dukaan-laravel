                                ...
                                @foreach ($accounts as $key => $account)
                                    <tr class="divide-x divide-gray-200 dark:divide-gray-700">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <p class="font-bold">Name: <span>{{ $account['firstname'].' '.$account['lastname']}}</span></p>
                                            <p class="font-bold">Mobile: <span>{{ $account['mobile']}}</span></p>
                                            <p class="font-bold">Email: <span>{{ $account['email']}}</span></p>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ config('constants.acc_types')[$account['type']] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $account['balance'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $account['created'] }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-center"><button type="button" class="text-gray-500 hover:text-gray-700 focus:outline-none"><i class="fas fa-eye"></i></button></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
